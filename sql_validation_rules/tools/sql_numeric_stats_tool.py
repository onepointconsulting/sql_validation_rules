...
class NumericColumnStatsSQLDatabaseTool(BaseSQLDatabaseTool, BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            numeric_columns = [
                {"name": c["name"]}
                for c in columns
                if str(c["type"]).startswith("DECIMAL")
                or str(c["type"]).startswith("NUMBER")
                or str(c["type"]).startswith("NUMERIC")
            ]
    ...
    async def _arun(
        ...
        raise NotImplementedError(
            "NumericColumnStatsSQLDatabaseTool does not support async"
        )
